...
def do_tclean_implementation_epoch(
    out_ms,
    year,
    min_freq,
    max_freq,
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT 1 FROM metadata WHERE tclean_epoch = ?", (outdir,))
    if cursor.fetchone():
        LOG.info(f"Skipping {outdir}, already in tclean_epoch of metadata DB.")
        return
    ...
    for ms_in in in_ms:
        update_metadata_column(db_path, "build_concat_epoch", ms_in, year, str(min_freq), str(max_freq), "tclean_epoch", outdir)
...
def main():
    ...
    if len(raw_args) < 3:
        print(
            "Usage: python do_tclean_implementation_epoch.py "
    ...
    # destringify the 14 tclean arguments
    ...
    try:
        do_tclean_implementation_epoch(*tclean_args)
    except Exception:
